...
void resend_keep_alive_auth_check_and_resend(Client *client, Logger *logger) {
  ...
  while (count_keep_alive_auth_send > count_keep_alive_auth_receive &&
         time(NULL) - time_keep_alive_auth_send > 2 &&
         count_retry_keep_alive_auth < 3) {
    socklen_t socklen_server_sockaddr =
        (socklen_t)sizeof(client->server_sockaddr);
    ssize_t size_send = -1;
    if (CONFIG_DEBUG) {
      debug_msg("resend keep alive auth to server",
                logger->keep_alive_auth_send, CONFIG_SIZE_KEEP_ALIVE_AUTH);
    }
    size_send = sendto(client->client_sockfd, logger->keep_alive_auth_send,
                       CONFIG_SIZE_KEEP_ALIVE_AUTH, 0,
                       (const struct sockaddr *)&(client->server_sockaddr),
...
void resend_keep_alive_heart_beat_check_and_resend(Client *client,
                                                   Logger *logger) {
  ...
  count_keep_alive_heart_beat_receive =
      logger->count_keep_alive_heart_beat_receive;
  ...
  while (count_keep_alive_heart_beat_send >
             count_keep_alive_heart_beat_receive &&
         time(NULL) - time_keep_alive_heart_beat_send > 2 &&
         count_retry_keep_alive_heart_beat < 3) {
    socklen_t socklen_server_sockaddr =
        (socklen_t)sizeof(client->server_sockaddr);
    ssize_t size_send = -1;
    if (CONFIG_DEBUG) {
      debug_msg("resend keep alive heartbeat to server",
                logger->keep_alive_heart_beat_send,
                CONFIG_SIZE_KEEP_ALIVE_HEART_BEAT);
    }
    size_send =
        sendto(client->client_sockfd, logger->keep_alive_heart_beat_send,
...
void *resend_run_keep_alive_auth_check_and_resend(void *args) {
  ...
  while (1) {
    ...
    resend_keep_alive_auth_check_and_resend(arg_resend->client,
                                            arg_resend->logger);
    sleep(1);
    ++count_turn;
  }
...
void *resend_run_keep_alive_heart_beat_check_and_resend(void *args) {
  ...
  while (1) {
    ...
    resend_keep_alive_heart_beat_check_and_resend(arg_resend->client,
                                                  arg_resend->logger);
    sleep(1);
    ++count_turn;
  }
