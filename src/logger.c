...
void logger_init(Logger *logger_this) {
  ...
  memset(logger_this->keep_alive_heart_beat_send, 0,
...
void logger_log_challenge_send(unsigned char *buffer_send) {
  if (CONFIG_DEBUG) {
    debug_msg("send challenge msg to server", buffer_send,
              CONFIG_SIZE_CHALLENGE);
  }
}
void logger_log_challenge_receive(unsigned char *buffer_receive,
                                  unsigned long size) {
  if (CONFIG_DEBUG) {
...
void logger_log_auth_receive(unsigned char *buffer_receive,
                             unsigned long size) {
  if (CONFIG_DEBUG) {
    debug_msg("receive auth msg from server", buffer_receive, size);
  }
}
void logger_log_keep_alive_auth_send(Logger *logger_this,
                                     unsigned char *buffer_send) {
  if (CONFIG_DEBUG) {
    debug_msg("send keep alive auth msg to server", buffer_send,
              CONFIG_SIZE_KEEP_ALIVE_AUTH);
  }
  logger_try_lock_mutex_keep_alive_auth_send(logger_this);
  memcpy(logger_this->keep_alive_auth_send, buffer_send,
         CONFIG_SIZE_KEEP_ALIVE_AUTH);
...
void logger_log_keep_alive_auth_receive(Logger *logger_this,
...
void logger_log_keep_alive_heart_beat_send(Logger *logger_this,
                                           unsigned char *buffer_send) {
  if (CONFIG_DEBUG) {
    debug_msg("send keep alive heart beat msg to server", buffer_send,
              CONFIG_SIZE_KEEP_ALIVE_HEART_BEAT);
  }
  logger_try_lock_mutex_keep_alive_heart_beat_send(logger_this);
  memcpy(logger_this->keep_alive_heart_beat_send, buffer_send,
         CONFIG_SIZE_KEEP_ALIVE_HEART_BEAT);
...
void logger_log_keep_alive_heart_beat_receive(Logger *logger_this,
                                              unsigned char *buffer_receive,
                                              unsigned long size) {
  if (CONFIG_DEBUG) {
    debug_msg("receive keep alive heart beat msg from server", buffer_receive,
              size);
  }
...
void logger_log_logout_receive(unsigned char *buffer_receive,
                               unsigned long size) {
  if (CONFIG_DEBUG) {
