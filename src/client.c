...
void client_init(Client *client_this) {
  ...
  memset(&(client_this->server_sockaddr), 0,
         sizeof(client_this->server_sockaddr));
  ...
  memset(client_this->server_drcom_indicator, 0,
         CONFIG_SIZE_SERVER_DRCOM_INDICATOR);
  client_this->count_heart_beat = 0;
  memset(client_this->heart_beat_server_token, 0,
         CONFIG_SIZE_HEART_BEAT_SERVER_TOKEN);
}
void client_connect(Client *client_this, const char *client_ip,
                    unsigned int client_port, const char *server_ip,
  ...
  if (bind(client_this->client_sockfd,
           (const struct sockaddr *)&client_sockaddr,
           socklen_client_sockaddr) < 0) {
  ...
  if (inet_pton(AF_INET, server_ip,
                &((client_this->server_sockaddr).sin_addr)) < 0) {
    perror("error when assigning server address");
    exit(EXIT_FAILURE);
  }
}
void client_challenge_login(Client *client_this, const char *auth_version,
                            Logger *logger) {
  ...
  socklen_t socklen_server_sockaddr =
      (socklen_t)sizeof(client_this->server_sockaddr);
  ssize_t size_send = -1, size_receive = -1;
  logger_log_challenge_send(buffer_send);
  size_send =
      sendto(client_this->client_sockfd, buffer_send, CONFIG_SIZE_CHALLENGE, 0,
             (const struct sockaddr *)&(client_this->server_sockaddr),
             socklen_server_sockaddr);
  if (size_send < 0) {
    perror("error when sending to server during login challenge");
    exit(EXIT_FAILURE);
  }
  size_receive =
      recvfrom(client_this->client_sockfd, buffer_receive, CONFIG_SIZE_BUFFER,
               0, (struct sockaddr *)&(client_this->server_sockaddr),
               &socklen_server_sockaddr);
  if (size_receive < 0) {
...
void client_login(Client *client_this, const char *username,
                  const char *password, const char *ip, const char *mac,
  ...
  unsigned char *md5a_plain =
      (unsigned char *)malloc(SIZE_MD5A_PLAIN * sizeof(unsigned char));
  md5a_plain[0] = 0x03;
  md5a_plain[1] = 0x01;
  ...
  // build MD5 digest b for password, md5(0x01 [salt] 0x00 0x00 0x00 0x00
  // [password])
  const int SIZE_MD5B_PLAIN = 9 + strlen(password);
  unsigned char *md5b_plain =
      (unsigned char *)malloc(SIZE_MD5B_PLAIN * sizeof(unsigned char));
  ...
  sscanf(ip, "%hhd.%hhd.%hhd.%hhd", &tmp_ip[0], &tmp_ip[1], &tmp_ip[2],
         &tmp_ip[3]);
  memcpy(buffer_send + 81, tmp_ip, 4); // [81:85)

  // concat "\x14\x00\x07\x0b" to first 97 bit, calculate MD5 digest c,
  // md5([first 97 bit] 0x14 0x00 0x07 0x0b)
  ...
  // ip dog 1 bit, delimiter 0x00 4 bit, default set by ``unsigned char
  // buffer_send[CONFIG_SIZE_BUFFER]={0}`` at the beginning
  ...
  sscanf(primary_dns, "%hhd.%hhd.%hhd.%hhd", &tmp_primary_dns[0],
         &tmp_primary_dns[1], &tmp_primary_dns[2], &tmp_primary_dns[3]);
  memcpy(buffer_send + 142, tmp_primary_dns, 4); // [142,146)

  // dhcp server 4 bit
  unsigned char tmp_dhcp_server[4] = {0};
  sscanf(dhcp_server, "%hhd.%hhd.%hhd.%hhd", &tmp_dhcp_server[0],
         &tmp_dhcp_server[1], &tmp_dhcp_server[2], &tmp_dhcp_server[3]);
  ...
  // unknown meaning fixed indicator 40 bit, same in gui drcom client across
  // different linux machine zero padding util 310th bit
  ...
  unsigned char *password_xor_md5a =
      (unsigned char *)malloc(SIZE_ROR_PASSWORD * sizeof(unsigned char));
  memset(password_xor_md5a, 0, SIZE_ROR_PASSWORD);
  encryption_gen_xor(md5a, (unsigned char *)password, 16, password_xor_md5a,
                     SIZE_ROR_PASSWORD);
  unsigned char *password_ror =
      (unsigned char *)malloc(SIZE_ROR_PASSWORD * sizeof(unsigned char));
  ...
  // checksum 4 bit
  const int SIZE_CHECKSUM_PLAIN =
      316 + SIZE_ROR_PASSWORD + 6 +
      6; // data (316 + SIZE_ROR_PASSWORD bit) + 0x01 0x26 0x07 0x11 0x00 0x00
         // (6 bit)+ mac (6 bit)
  unsigned char *checksum_plain =
      (unsigned char *)malloc((SIZE_CHECKSUM_PLAIN) * sizeof(unsigned char));
  ...
  // zero padding, default set by ``unsigned char
  // buffer_send[CONFIG_SIZE_BUFFER]={0}`` at the beginning
  ...
  socklen_t socklen_server_sockaddr =
      (socklen_t)sizeof(client_this->server_sockaddr);
  ssize_t size_send = -1, size_receive = -1;
  size_send = sendto(client_this->client_sockfd, buffer_send, SIZE_LOGIN, 0,
                     (const struct sockaddr *)&(client_this->server_sockaddr),
                     socklen_server_sockaddr);
  if (size_send < 0) {
...
void client_keep_alive_auth(Client *client_this, Logger *logger,
  ...
  socklen_t socklen_server_sockaddr =
      (socklen_t)sizeof(client_this->server_sockaddr);
  ssize_t size_send = -1, size_receive = -1;
  size_send = sendto(client_this->client_sockfd, buffer_send,
                     CONFIG_SIZE_KEEP_ALIVE_AUTH, 0,
                     (const struct sockaddr *)&(client_this->server_sockaddr),
                     socklen_server_sockaddr);
  if (size_send < 0) {
...
void *client_run_keep_alive_auth(void *args) {
  ...
  unsigned char buffer_receive[CONFIG_SIZE_BUFFER] = {0};
  while (1) {
    client_keep_alive_auth(arg_keep_alive_auth->client,
                           arg_keep_alive_auth->logger, buffer_send,
                           buffer_receive);
    sleep(20);
  }
}
void client_keep_alive_heart_beat(
    Client *client_this, const char *keep_alive_heart_beat_version,
    const char *keep_alive_first_heart_beat_version,
    const char *keep_alive_extra_heart_beat_version, const char *ip,
    Logger *logger, unsigned char *buffer_send, unsigned char *buffer_receive) {
  // first heart beat
  if (client_this->count_heart_beat == 0) {
    ...
    ssize_t size_send = -1, size_receive = -1;
    size_send = sendto(client_this->client_sockfd, buffer_send,
                       CONFIG_SIZE_KEEP_ALIVE_HEART_BEAT, 0,
                       (const struct sockaddr *)&(client_this->server_sockaddr),
                       socklen_server_sockaddr);
    if (size_send < 0) {
    ...
    logger_log_keep_alive_heart_beat_receive(logger, buffer_receive,
                                             size_receive);
  ...
  if (client_this->count_heart_beat % 21 == 0) {
    ...
    memcpy(buffer_send + 16, client_this->heart_beat_server_token,
           4); // [16:20)
...
void *client_run_keep_alive_heart_beat(void *args) {
  ARG_KeepAliveHeartBeat *arg_keep_alive_heart_heat =
      (ARG_KeepAliveHeartBeat *)args;
  unsigned char buffer_send[CONFIG_SIZE_KEEP_ALIVE_HEART_BEAT] = {0};
  unsigned char buffer_receive[CONFIG_SIZE_BUFFER] = {0};
  while (1) {
    client_keep_alive_heart_beat(
        arg_keep_alive_heart_heat->client,
        ...
        arg_keep_alive_heart_heat->ip, arg_keep_alive_heart_heat->logger,
        buffer_send, buffer_receive);
    sleep(20);
  }
}
void client_challenge_logout(Client *client_this, const char *auth_version,
                             Logger *logger) {
  unsigned char buffer_send[CONFIG_SIZE_CHALLENGE] = {0};
  unsigned char buffer_receive[CONFIG_SIZE_BUFFER] = {0};
  /*start build challenge packet*/
  // challenge packet begins with 0x01 0x03, which is the only difference from
  // challenge_login()
  buffer_send[0] = 0x01;
  buffer_send[1] = 0x03;
  // two random numbers
...
void client_logout(Client *client_this, const char *username,
                   const char *password, const char *mac,
                   const char *auth_version, const char *control_check_status,
                   const char *adapter_num, Logger *logger) {
  ...
  ssize_t size_send = -1, size_receive = -1;
  size_send =
      sendto(client_this->client_sockfd, buffer_send, CONFIG_SIZE_LOGOUT, 0,
             (const struct sockaddr *)&(client_this->server_sockaddr),
             socklen_server_sockaddr);
  if (size_send < 0) {
