...
void encryption_gen_md5(const void *data, unsigned long size_input,
                        unsigned char *result) {
  MD5(data, size_input, result);
}
void encryption_gen_xor(unsigned char *data_a, unsigned char *data_b,
                        unsigned long size_input, unsigned char *result,
...
void encryption_gen_ror(unsigned char *data, unsigned long size,
                        unsigned char *result) {
  for (unsigned long i = 0; i < size; ++i) {
    result[i] = (unsigned char)((data[i] << 3) + (data[i] >> 5));
  }
}
void encryption_gen_checksum(unsigned char *data, unsigned long size_input,
                             unsigned char *result, unsigned long size_output) {
...
void encryption_gen_crc(unsigned char *data, unsigned long size_input,
                        unsigned char *result, unsigned long size_output) {
